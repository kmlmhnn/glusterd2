...
func glusterdVolInfoResetStats(r *rebalanceapi.RebalanceInfo) {
	r.RebalanceFiles = 0
	r.RebalanceData = 0
	r.LookedupFiles = 0
...
func setCommitHash(commit uint64) uint64 {
...
// GetRebalanceInfo provides stored rebalance detaials
func GetRebalanceInfo(Volname string) (*rebalanceapi.RebalanceInfo, error) {
	...
	if resp.Count != 1 {
		log.WithField("volume", Volname).Error("Rebalance info not found for the volume")
		return nil, errors.New("Rebalance info not found for the volume")
	}
