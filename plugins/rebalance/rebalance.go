...
type RebalanceProcess struct {
	...
	pidfilepath    string
	rebalance      rebalanceapi.RebalanceInfo
}
...
func (r *RebalanceProcess) Args() string {
	...
	volfileserver, _, _ := net.SplitHostPort(config.GetString("clientaddress"))
	if volfileserver == "" {
	...
	buffer.WriteString(fmt.Sprintf(" -s %s", volfileserver))
...
func NewRebalanceProcess(rinfo rebalanceapi.RebalanceInfo) (*RebalanceProcess, error) {
...
func (r *RebalanceProcess) ID() string {
	return "rebalance-" + r.rebalance.Volname
}
